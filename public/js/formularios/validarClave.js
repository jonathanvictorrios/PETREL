...
const testPassw = clave => {
    ...
    /* Mis regex, en orden:
    * Débil: Debe incluir solo números o solo letras, mínimo 4 caracteres
    * Normal: Debe incluir números y letras, mínimo 6 caracteres
    * Fuerte: Debe incluir números, letras y símbolos, mínimo 8 caracteres
    * Insegura: Incluye menos de 4 caracteres
    */
    const debil = /(?=.*[a-zñáéíóúüÁÉÍÓÚÜÑ]).{4,}|(?=.*[\d]).{4,}/i;
    const normal = /(?=.*[a-zñáéíóúüÁÉÍÓÚÜÑ])(?=.*[\d]).{6,}/i;
    const fuerte = /(?=.*[a-zñáéíóúüÁÉÍÓÚÜÑ])(?=.*[\d])(?=.*[\!@#$%^&*()\\[\]{}\-_+=~`|:;"'<>,.\/?¿¡]).{8,}/i;
...
/* Función del botón mostrar texto de contraseña:
